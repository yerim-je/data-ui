            ...
            <div id="rec">
                ...
                <div class="slider">
                    <div class="slide-container">
                        <div class="slide" v-for="(movie, index) in recList" :key="index"
                        @mouseover="handleMouseOver(movie)" @mouseout="handleMouseOut(movie)">
                            <RouterLink :to="{ name: 'MovieTitle', query: { movieName: movie.제목 } }">
                                <img class="poster" :src="movie.포스터" :alt="movie.제목">
                                <div class="summary">{{ movie.줄거리 }}</div>
                            </RouterLink>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    setup() {
        ...
        //     { imgSrc: '../src/images/img/눈물이주룩주룩.jpg', altText: '눈물이 주룩주룩' ,text:'레스토랑을 차리는 꿈을 향해 성실하게 하루하루를 지내던 21살의 요타로(츠마부키 사토시 분). 8살 난 꼬마였던 그 때, 어느 날 갑자기 "여동생"이 생겼다. 그 후 그의 생애를 희망으로 가득 채웠던 그녀가 고등학교에 입학하며 그와 한 집에 살겠다고 찾아왔다.  고등학교에 입학하며 오빠 집에서 같이 살게 된 여고생 카오루(나가사와 마사미 분). 하지만 그녀는 아무에게도 말하지 못한 비밀이 있다. 오빠는 그녀가 너무 어려서 기억하지 못할거라 믿고 있었지만, 그녀는 기억하고 있다. 그가 친오빠가 아니라는 것을. 오키나와의 눈부신 태양과 투명한 바다, 향기로운 바람, 색색깔의 꽃들. 평소 그냥 지나치던 모든 것들이 둘이 함께 하면서 더욱 아름답게 다가온다. 하지만, 하고 싶은 말을 마음 속으로만 삼키면서도 행복했던 그들의 시간을 방해하는 사건들이 벌어지는데.'},
        //     { imgSrc: '../src/images/img/모노노케히메.jpg', altText: '모노노케 히메' ,text:''},
